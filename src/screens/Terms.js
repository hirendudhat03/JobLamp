...
class TermsConditions extends Component
{
    ...
    getTermsConditionsAPICall()
    {
        this.setState({showloading:true})

        fetch(API_ROOT + 'terms-conditions', {
            method: 'get',
            headers: {
            ...
            .then((responseData) => {
                ...
                if (responseData.status == true) {
                    //this.setState({showloading:false,terms_conditions:responseData.data.content_en})
                    ...
                    {
                        this.setState({showloading:false,terms_conditions:responseData.data.content_es})
                    }
                    else if(global.language == 'fr')
                    {
                        this.setState({showloading:false,terms_conditions:responseData.data.content_fr})
                    }
                    else if(global.language == 'de')
                    {
                        this.setState({showloading:false,terms_conditions:responseData.data.content_de})
                    }
                    else if(global.language == 'pt')
                    {
                        this.setState({showloading:false,terms_conditions:responseData.data.content_pt})
                    }
                    else if(global.language == 'ru')
                    {
                        this.setState({showloading:false,terms_conditions:responseData.data.content_ru})
                    }
                } else {
    ...
    renderTopBar()
    {
        return (
            <View style = {{backgroundColor:'#c80025', height:80, width:'100%', flexDirection:'row', justifyContent:'center'}}>
                    ...
                    <Text style = {{marginTop:45, width:'70%', height:35,textAlign:'center', color:'#fff', fontSize:20, fontWeight:'400'}}>{strings.TermsConditions}</Text>
            </View>
        );
    }
        ...
        return (
            <View style = {{flex:1,backgroundColor:'#fff',alignItems:'center'}}>
                ...
                <View style = {{width:'100%',height:'90%',backgroundColor:'red'}}>
                    <WebView source={{ html:this.state.terms_conditions}} />
                </View>
