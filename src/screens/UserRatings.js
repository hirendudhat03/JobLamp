        ...
        fetch(API_ROOT + 'review-list', {
            ...
            .then((responseData) => {
                if (responseData.status == true) {
                    ...
                    this.setState({showloading:false, arrratings:responseData.data})
    ...
    renderTopBar()
    {
        return (
            <View style = {{backgroundColor:'#c80025', height:80, width:'100%', flexDirection:'row', justifyContent:'center'}}>
                    ...
                    <Text style = {{marginTop:45, width:'70%', height:35,textAlign:'center', color:'#fff', fontSize:20, fontWeight:'600'}}>{strings.Reviews}</Text>
            ...
            <View style = {{flex:1, backgroundColor:'#f0f0f0'}}>
                {this.renderTopBar()}
                {
                    ...
                    <FlatList
                        ...
                        data={this.state.arrratings}
                        ref={(ref) => { this.scroll = ref; }}
                        renderItem={({ item, index }) => {
                            return (
                                <View style={{ borderRadius: 5,padding: 10,backgroundColor: 'transparent'}}>
                                    <View style = {{backgroundColor:'#fff', width:deviceWidth - 30,alignItems:'center',borderRadius:10, paddingVertical:10, flexDirection:'row'}}>
                                        ...
                                        <View style = {{justifyContent:'center',width:'75%', marginLeft:10}}>
                                            ...
                                            <Text style = {{fontSize:12,fontWeight:'300',color:'#6e6e6e'}} numberOfLines = {1}>{item.feedback}</Text>
