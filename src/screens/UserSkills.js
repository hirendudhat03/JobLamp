...
class UserSkills extends Component
{
    constructor (props)
    {
        ...
        this.state = {
            arrrskills:[],
            device_token: '',
            user_skills:props.navigation.state.params.user_skills,
            selected_indexes:[],
            showloading:false,
    ...
    onPressedSkill(index)
    {
        ...
        else
        {
            ...
            {
                alert(strings.UpgradePlanForSkill)
            }
            else
            {
                var indexcopy = this.state.selected_indexes
    ...
    getUserSkillsAPICall()
    {
    
        this.setState({showloading:true})

        fetch(API_ROOT + 'get-skill', {
            method: 'get',
            headers: {
                ...
                if (responseData.status == true) {
                    ...
                    if(skills != undefined && skills != null)
                    {
                        for (var i = 0; i < skills.length; i++) {
                            ...
                            {
                                if(this.state.user_skills.some(objcat => objcat.name_en === obj.name_en))
                                {
                                    indexes.push(i)
                                }
                            }
                            ...
                            {
                                if(this.state.user_skills.some(objcat => objcat.name_fr === obj.name_fr))
                                {
                                    indexes.push(i)
                                }
                            }
                            ...
                            {
                                if(this.state.user_skills.some(objcat => objcat.name_es === obj.name_es))
                                {
                                    indexes.push(i)
                                }
                            }
                            ...
                            {
                                if(this.state.user_skills.some(objcat => objcat.name_de === obj.name_de))
                                {
                                    indexes.push(i)
                                }   
                            }
                            ...
                            {
                                if(this.state.user_skills.some(objcat => objcat.name_pt === obj.name_pt))
                                {
                                    indexes.push(i)
                                }
                            }
                            ...
                            {
                                if(this.state.user_skills.some(objcat => objcat.name_ru === obj.name_ru))
                                {
                                    indexes.push(i)
                                }
                            }
                    ...
                    this.setState({showloading:false, arrrskills:responseData.data,selected_indexes:indexes})
    ...
    updateSkillAPICall()
    {
        ...
        for (var i = 0; i < this.state.selected_indexes.length; i++) {
            ...
            let objskill = this.state.arrrskills[idx]
            let skillid = objskill.id
            arrskillids.push(skillid)
        }
        ...
        data.append('skills', JSON.stringify(arrskillids))

        fetch(API_ROOT + 'update-skill', {
            method: 'post',
            body:data,
            headers: {
              Authorization: 'Bearer ' + this.state.device_token,
    ...
    renderTopBar()
    {
        return (
            <View style = {{backgroundColor:'#c80025', height:80, width:'100%', flexDirection:'row', justifyContent:'center'}}>
                    ...
                    <Text style = {{marginTop:45, width:'70%', height:35,textAlign:'center', color:'#fff', fontSize:20, fontWeight:'600'}}>{strings.Skills}</Text>
